...
//    Creating a pointcut
    ...
    @Pointcut("execution(* com.aop.application.repository.*.get*(..))")
    private void getPointCut(){}
    @Pointcut("execution(* com.aop.application.repository.*.set*(..))")
    private void setPointCut(){}


//    applying the pointcut
    @Before("mainPointCut()&&!(getPointCut() || setPointCut())")
    public void pointcutApplied(){
        System.out.println("===> LISTENING EXCEPT ON GETTER AND SETTERS<===");
    }
